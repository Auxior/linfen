...
export const AddPoylineMarker = async (val: number, status: boolean) => {
    ...
    if (status && val) {
        ...
        let arr = [29.150436401367188, 33.95029830932617, 38.19725036621094, 42.68795394897461, 47.09477615356445, 51.631778717041016, 56.138301849365234, 60.60261154174805, 65.09162902832031, 69.84569549560547, 74.23262786865234, 78.71785736083984, 83.10263061523438, 87.59577178955078, 89.31773376464844, 96.7956314086914, 101.10557556152344, 105.837158203125, 110.22123718261719, 114.78348541259766, 119.31388854980469, 123.78229522705078, 128.37246704101562, 132.80445861816406, 137.3146514892578, 141.84031677246094, 146.3065185546875, 150.81942749023438, 155.34146118164062, 159.83111572265625, 164.39207458496094, 168.740234375, 173.2401580810547, 177.72328186035156, 182.2175750732422]
        let height: any = arr[val - 1]
        const Os: any = []
        const Ms: any = []
        let camera = [32.092046, 496.282266, 87.249482, -59.045696, 21.079615, 1]
        Object.keys(Coords).map((item: any, index) => {
            const features = featureCollection(Coords[item].coord.map((item: Position) => point(item)))
            ...
            Ms.push({
                ...
                pointName: Random % 3 === 1 ? 'Point_R_3' : Random % 4 === 1 ? 'Point_Y_3' : 'Point_B_4', //3D标注展示的特效名称
                pRandomointVisible: true, //3D标注是否显示
                pointScale: 0.8, //3D标注整体缩放比例
                coordinate: [centers.geometry.coordinates[0], centers.geometry.coordinates[1], height + 2.3] //3D标注的位置坐标
            })
        })
        await __g.marker3d.add(Ms)
        await __g.polygon.add(Os)
        const copyData = JSON.parse(JSON.stringify(camera))
        const cameraArr = [copyData[0], copyData[1], copyData[2] + (val - 1) * 4, copyData[3], copyData[4], copyData[5]]
        await __g.camera.set(cameraArr)
    } else {
        await __g.polygon.clear()
        await __g.marker3d.clear()
    }
