...
const polyLineCoordinates2 = [startPoint2, [71.73602294921875, 479.8523864746094, 112.01499938964844], [67.18951416015625, 472.5605163574219, 112.5149917602539], [49.10723114013672, 482.8077697753906, 112.01499938964844], [48.10822296142578, 480.8288269042969, 112.01499938964844], [43.15809631347656, 483.66925048828125, 112.01499938964844], [44.51614761352539, 485.7375793457031, 112.01499938964844], [39.59901428222656, 488.739013671875, 112.01502227783203], [39.98722839355469, 489.5272216796875, 112.01498413085938], [45.9306640625, 485.9652099609375, 112.01499938964844], [44.51201629638672, 483.5860290527344, 112.01499938964844], [47.57404708862305, 481.9135437011719, 112.01499938964844], [52.1097526550293, 490.3991394042969, 112.01499938964844], endPoint2]
const drawPersonnelTrack = async (type: string, id: number) => {
    ...
    if (type === '人员黑名单') {
        ...
        __g.tileLayer.hide(['jz01', 'jz02', 'jz03', 'jz04', 'jz05', 'jz06', 'jz07', 'jz08', 'jz09', 'jz10', 'jz11', 'jz12', 'jz13', 'jz14', 'jz15', 'jz16', '灯光'].map(p => AirCityStore.layerTreeObject[p]))
        __g.tileLayer.setStyle(AirCityStore.layerTreeObject['jz17'], 0, '#fff', 1, 0.32, 1, 0.18)
        if (id === 617) {
            ...
            __g.camera.set(38.184498, 485.557739, 149.399918, -65.061615, 9.821373, 1)
        } else {
            __g.camera.set(58.522614, 508.245667, 134.447937, -59.933037, 73.39299, 1)
        }
        //起始
        ...
        await __g.tileLayer.show(['jz01', 'jz02', 'jz03', 'jz04', 'jz05', 'jz06', 'jz07', 'jz08', 'jz09', 'jz10', 'jz11', 'jz12', 'jz13', 'jz14', 'jz15', 'jz16', 'jz17', 'jz18', 'jz19', 'jz20', 'jz21', 'jz22', 'jz23', 'jz24', 'jz25', 'jz26', 'jz27', 'jz28', 'jz29', 'jz30', 'jz31', 'jz32', 'jz33', 'jz34', 'jz35'].map(p => AirCityStore.layerTreeObject[p]))
        // ChaiLouStore.ClickLayer(20, true);
        await __g.tileLayer.hide(['jz01', 'jz02', 'jz03', 'jz04', 'jz05', 'jz06', 'jz07', 'jz08', 'jz09', 'jz10', 'jz11', 'jz12', 'jz13', 'jz14', 'jz15', 'jz16', '灯光'].map(p => AirCityStore.layerTreeObject[p]))

        __g.tileLayer.setStyle(AirCityStore.layerTreeObject['jz17'], 0, '#fff', 1, 0.32, 1, 0.18)

        const bbox = [30.93803787231445, 401.3389892578125, 112.06080932617, 113.13697052001953, 533.01446533203125, 112.067686080932617]

        await __g.decal.clear()
        await __g.decal.add({
            id: 'd2',
            decalBlendMode: 0, //贴花类型，取值范围：[0,1]；取值说明：0背景混合，1仅贴花；默认值：0
            order: 1, //贴花层级，值越大层级越高，层级最高的显示在最上面一层
            texturePath: `${window.location.origin}/img/heatMap_download0.png`, //贴图文件路径
            rotation: [1, 0, 0],
            location: [62.202049255371094, 487.7946472167969, 112.01548],
            bbox: bbox
        })
        __g.camera.set([58.80978, 506.978867, 149.598145, -68.165359, 68.43721, 1])
        // const range = [0, 100];
        // const data: any = [];
        // for (let i = 0; i < 20; i++) {
        ...
        //         radius: Math.random() * 8000, //热力点影像半径范围
        //         heatValue: Math.random() * 200, //热力值
        //     });
        // }
...
export { defaultRadiationPointOpt, defaultPolyLineOpt, default3DMarkOpt, defaultTagOpt, drawPersonnelTrack, getRandNumBetween }
